...
class VscAccountPageVo:
    ...
    def vo(self):
        if not self._vo_cache:
            self._vo_cache = mkVo(
                whenHTTPErrorRaise(
                    self.rest_client.vo[self.vo_id].get, f"Could not get VO from accountpage for VO {self.vo_id}"
                )[1]
            )
...
class VscTier2AccountpageVo(VscAccountPageVo, VscTier2Accountpage):
    ...
    def _institute_quota(self):
        if not self._institute_quota_cache:
            all_quota = [
                mkVscVoSizeQuota(q)
                for q in whenHTTPErrorRaise(
                    self.rest_client.vo[self.vo.vsc_id].quota.get,
                    f"Could not get quota from accountpage for VO {self.vo.vsc_id}",
                )[1]
            ]
    ...
    def _get_institute_non_shared_data_quota(self):
        return [
            q.hard for q in self._get_institute_data_quota() if not q.storage["name"].endswith(STORAGE_SHARED_SUFFIX)
        ]

    def _get_institute_shared_data_quota(self):
        return [q.hard for q in self._get_institute_data_quota() if q.storage["name"].endswith(STORAGE_SHARED_SUFFIX)]
    ...
    def vo_scratch_quota(self):
        if not self._vo_scratch_quota_cache:
            self._vo_scratch_quota_cache = [
                q for q in self._institute_quota if q.storage["storage_type"] == SCRATCH_KEY
            ]
    ...
    def sharing_group(self):
        ...
        if not self._sharing_group_cache:
            group_name = self.vo.vsc_id.replace(
            ...
            self._sharing_group_cache = mkVscAutogroup(
                whenHTTPErrorRaise(
                    self.rest_client.autogroup[group_name].get, f"Could not get autogroup {group_name} details"
                )[1]
            )
    ...
    def set_scratch_quota(self, storage_name):
        ...
        if not quota:
            ...
            logging.info(
                "Setting default VO %s scratch quota on storage %s to %d",
                self.vo.vsc_id,
                storage_name,
                self.storage[storage_name].quota_vo,
            )
            self._set_quota(storage_name, self._scratch_path(storage_name), self.storage[storage_name].quota_vo)
            return
        elif len(quota) > 1:
            logging.exception("Cannot set scratch quota for %s with multiple quota instances %s", storage_name, quota)
            raise
    ...
    def set_member_data_quota(self, member):
        ...
        if not self.vo_data_quota:
            logging.warning(
                "Not setting VO %s member %s data quota: no VO data quota info available",
                self.vo.vsc_id,
                member.account.vsc_id,
            )
            return

        if self.vo.vsc_id in DEFAULT_VOS_ALL:
            logging.warning(
                "Not setting VO %s member %s data quota: No VO member quota for this VO",
                member.account.vsc_id,
                self.vo.vsc_id,
            )
            return
        ...
        if member.vo_data_quota:
            ...
            quota = [
                q
                for q in member.vo_data_quota
                if q.fileset == self.vo.vsc_id and not q.storage["name"].endswith(STORAGE_SHARED_SUFFIX)
            ]
            if len(quota) > 1:
                logging.exception("Cannot set data quota for member %s with multiple quota instances %s", member, quota)
                raise
            else:
                logging.info(
                    "Setting the data quota for VO %s member %s to %d KiB",
                    self.vo.vsc_id,
                    member.account.vsc_id,
                    quota[0].hard,
                )
    ...
    def set_member_scratch_quota(self, storage_name, member):
        ...
        if not self.vo_scratch_quota:
            logging.warning(
                "Not setting VO %s member %s scratch quota: no VO quota info available",
                self.vo.vsc_id,
                member.account.vsc_id,
            )
            return
        ...
        if self.vo.vsc_id in DEFAULT_VOS_ALL:
            logging.warning(
                "Not setting VO %s member %s scratch quota: No VO member quota for this VO",
                member.account.vsc_id,
                self.vo.vsc_id,
            )
            return
        ...
        if member.vo_scratch_quota:
            quota = [
                q
                for q in member.vo_scratch_quota
                if q.storage["name"] in (storage_name,) and q.fileset in (self.vo_id,)
            ]
            if quota:
                logging.info(
                    "Setting the scratch quota for VO %s member %s to %d GiB on %s",
                    self.vo.vsc_id,
                    member.account.vsc_id,
                    quota[0].hard / 1024 / 1024,
                    storage_name,
                )
                self._set_member_quota(storage_name, self._scratch_path(storage_name), member, quota[0].hard)
            else:
                logging.error(
                    "No VO %s scratch quota for member %s on %s after filter (all %s)",
                    self.vo.vsc_id,
                    member.account.vsc_id,
                    storage_name,
            ...
            logging.error(
                "No VO %s scratch quota set for member %s on %s", self.vo.vsc_id, member.account.vsc_id, storage_name
            )
    ...
    for vo_id in sorted(vo_ids):
        ...
        try:
            ...
            factory = lambda vid: VscTier2AccountpageUser(
                vid, rest_client=client, host_institute=host_institute, use_user_cache=True
            )
